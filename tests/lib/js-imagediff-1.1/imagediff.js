...
(function (name, definition) {
  ...
  function toImageDataFromCanvas (canvas) {
    var
      ...
      context = canvas.getContext('2d');
    // DeNA MOD.
    // Wahid sometimes uses WebGLRenderingContext instead of
    // CanvasRenderingContext2D, then 'context' will be null.
    if (context) {
      return context.getImageData(0, 0, width, height);
    } else {
      // If Wahid is using WebGLRenderingContext,
      // copies image data to a temporary canvas to compare.
      ...
      if (gl) {
        ...
        var newCanvas = getCanvas(width, height);
        var context2d = newCanvas.getContext('2d');
        ...
        for (var y=height-1; y>=0; --y) {
          for (var x=0; x<width; ++x) {
            var pixel = width * y + x;
            data[i++] = buffer[pixel*4+0];
						data[i++] = buffer[pixel*4+1];
            data[i++] = buffer[pixel*4+2];
						data[i++] = buffer[pixel*4+3];
          }
        }
				// Default WebGLRenderingContext is premultiplied alpha mode,
				// CanvasRenderingContext2D is (almost) straight alpha.
	  		imageData.premultipliedAlpha = true;
        return imageData;
      }
    }
    return null;
  ...
  function toCanvas (object) {
	...
	// DeNA MOD.
  // Default alpha mode of WebGLRenderingContext and
  // CanvasRenderingContext2D are insame...
  // So make uniform format to compare them.
	function convertToPremultipliedAlpha(imageData) {
		if (imageData.premultipliedAlpha) {
			return;
		}

		var data = imageData.data;
		if (data.length % 4) {
			console.assert('The image data format is not RBGA:'+imageData);
			return;
		}
		...
		for (var i=0; i<pixels; ++i) {
			var p = i * 4;
			var alpha = data[p+3] / 255.0;
			data[p+0] = (data[p+0] * alpha + 0.5) | 0;
			data[p+1] = (data[p+1] * alpha + 0.5) | 0;
			data[p+2] = (data[p+2] * alpha + 0.5) | 0;
		}
		imageData.premultipriedAlpha = true;
  	return;
	}
  ...
  function equal (a, b, tolerance) {
    ...
    if (!equalDimensions(a, b)) return false;
		...
		// DeNA MOD.
    // Make uniform alpha mode to compare them.
		if (a.premultipliedAlpha != b.premultipliedAlpha) {
			convertToPremultipliedAlpha(a);
			convertToPremultipliedAlpha(b);
		}

		for (i = length; i--;) {
			if (aData[i] !== bData[i] && Math.abs(aData[i] - bData[i]) > tolerance) {
				//var p = i / 4 | 0;
				//console.log('diff['+i+'('+(p%32)+','+(p/32|0)+':'+(i%4)+')]: '+ Math.abs(aData[i] - bData[i]));
				return false;
			}
    }
    return true;
  ...
  function diffEqual (a, b, options) {
    ...
    for (i = 0; i < length; i += 4) {
      ...
      // DeNA MOD.  To make easy to check diffs.
      var amp = 16;
			cData[i] *= amp;
			cData[i+1] *= amp;
			cData[i+2] *= amp;
  ...
  function imageDiffEqualMessage (actual, expected) {
    return function () {
      ...
      var
        div     = get('div'),
        text    = get('div', '<span style="color:yellow;'
                           + ' background-color:black">'
                           + 'Due to optimization for mobile games, '
                           + 'this spec may fail.  '
                           + 'Consider whether the diff is acceptable or not'
                           + ' by manual.</span>' ),
